...
tape("Data Graph Integration Tests", assert => {
  ...
  assert.deepEqual(state[1], chartNode1.getState(), "chartNode1 state should equal the corresponding graph state slice")
  assert.deepEqual(state[2], chartNode2.getState(), "chartNode2 state should equal the corresponding graph state slice")
  assert.deepEqual(state["global"], globalFilterNode.getState(), "globalFilterNode state should equal the corresponding graph state slice")
  assert.deepEqual(state["xfilter"], xFilterNode.getState(), "xFilterNode state should equal the corresponding graph state slice")
