(function (global, factory) {
	typeof exports === 'object' && typeof module !== 'undefined' ? factory(exports, require('vega-util'), require('vega-dataflow'), require('vega-expression'), require('d3-color'), require('d3-array'), require('d3-format'), require('d3-time-format'), require('vega-scale'), require('vega-scenegraph'), require('vega-event-selector')) :
	typeof define === 'function' && define.amd ? define(['exports', 'vega-util', 'vega-dataflow', 'vega-expression', 'd3-color', 'd3-array', 'd3-format', 'd3-time-format', 'vega-scale', 'vega-scenegraph', 'vega-event-selector'], factory) :
	(factory((global.vega = global.vega || {}),global.vega,global.vega,global.vega,global.d3,global.d3,global.d3,global.d3,global.vega,global.vega,global.vega));
}(this, (function (exports,vegaUtil,vegaDataflow,vegaExpression,d3Color,d3Array,d3Format,d3TimeFormat,vegaScale,vegaScenegraph,vegaEventSelector) { 'use strict';
...
function panLinear(domain, delta) {
  return pan(domain, delta, vegaUtil.toNumber, vegaUtil.identity);
}
...
function zoomLinear(domain, anchor, scale) {
  return zoom(domain, anchor, scale, vegaUtil.toNumber, vegaUtil.identity);
}
...
function fieldRef(data, scope) {
  ...
  if (vegaUtil.isArray(data)) {
    ...
    scope.signalRef('setdata(' + vegaUtil.stringValue(name) + ',' + data.signal + ')');
  }
...
var defaults = function(configs) {
  ...
  (configs || []).forEach(function(config) {
    ...
    if (config) for (key in config) {
      output[key] = vegaUtil.isObject(output[key])
        ? vegaUtil.extend(output[key], config[key])
