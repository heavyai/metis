...
function parseParameters(spec, ctx, params) {
  ...
  for (key$$1 in spec) {
    ...
    params[key$$1] = vegaUtil.isArray(value)
...
function getKey(_, ctx) {
  var k = 'k:' + _.$key;
  return ctx.fn[k] || (ctx.fn[k] = vegaUtil.key(_.$key));
}
 ...
 */
var parseStream = function(spec, ctx) {
 ...
 */
var parseUpdate = function(spec, ctx) {
...
function setState(state) {
  ...
  Object.keys(signals || {}).forEach(function(key$$1) {
    df.update(ctx.signals[key$$1], signals[key$$1], SKIP);
  });

  Object.keys(data || {}).forEach(function(key$$1) {
    df.pulse(
      ctx.data[key$$1].input,
      df.changeset().remove(vegaUtil.truthy).insert(data[key$$1])
    );
  });
 ...
 */
var context = function(df, transforms, functions) {
