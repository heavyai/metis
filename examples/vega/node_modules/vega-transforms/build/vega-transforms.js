(function (global, factory) {
	...
	(factory((global.vega = global.vega || {}, global.vega.transforms = global.vega.transforms || {}),global.vega,global.vega,global.vega,global.d3));
...
function TupleStore(key$$1) {
  this._key = key$$1 ? vegaUtil.field(key$$1) : vegaDataflow.tupleid;
  this.reset();
}
...
var prototype = vegaUtil.inherits(Aggregate, vegaDataflow.Transform);
...
prototype.cell = function(key$$1, t) {
...
prototype.newcell = function(key$$1, t) {
  var cell = {
    key:   key$$1,
    num:   0,
    agg:   null,
...
var SortedList = function(idFunc, source, input) {
  ...
  return {
    ...
    data: function(compare$$1, resort) {
      ...
      if (add.length) {
        data = compare$$1
          ? vegaUtil.merge(compare$$1, data, add.sort(compare$$1))
          : data.concat(add);
        add = [];
      }
      return data;
...
function Extent(params) {
  vegaDataflow.Transform.call(this, [+Infinity, -Infinity], params);
}
...
var prototype$8 = vegaUtil.inherits(Facet, vegaDataflow.Transform);

prototype$8.activate = function(flow) {
  this._targets[this._targets.active++] = flow;
};

prototype$8.subflow = function(key$$1, flow, pulse, parent) {
...
var prototype$10 = vegaUtil.inherits(Filter, vegaDataflow.Transform);
...
var prototype$11 = vegaUtil.inherits(Fold, vegaDataflow.Transform);
...
prototype$11.transform = function(_, pulse) {
  ...
  function add(t) {
    var f = (cache[vegaDataflow.tupleid(t)] = Array(n)); // create cache of folded tuples
    for (var i=0, ft; i<n; ++i) { // for each key, derive folds
      ft = (f[i] = vegaDataflow.derive(t));
      ft[key$$1] = keys[i];
      ft[value] = fields[i](t);
      out.add.push(ft);
    }
  ...
  function mod(t) {
    var f = cache[vegaDataflow.tupleid(t)]; // get cache of folded tuples
    for (var i=0, ft; i<n; ++i) { // for each key, rederive folds
      if (!(mask & (1 << i))) continue; // field is unchanged
      ft = vegaDataflow.rederive(t, f[i], stamp);
      ft[key$$1] = keys[i];
      ft[value] = fields[i](t);
      out.mod.push(ft);
    }
...
var prototype$12 = vegaUtil.inherits(Formula, vegaDataflow.Transform);
...
var prototype$13 = vegaUtil.inherits(Generate, vegaDataflow.Transform);
...
var prototype$14 = vegaUtil.inherits(Impute, vegaDataflow.Transform);
...
var prototype$15 = vegaUtil.inherits(JoinAggregate, Aggregate);
...
var prototype$16 = vegaUtil.inherits(Lookup, vegaDataflow.Transform);
...
var prototype$17 = vegaUtil.inherits(PreFacet, Facet);
...
prototype$17.transform = function(_, pulse) {
  ...
  if (_.modified('field') || field$$1 && pulse.modified(vegaUtil.accessorFields(field$$1))) {
  ...
  pulse.visit(pulse.ADD, function(t) {
    var sf = self.subflow(vegaDataflow.tupleid(t), flow, pulse, t);
    field$$1 ? field$$1(t).forEach(function(_) { sf.add(vegaDataflow.ingest(_)); }) : sf.add(t);
  });
...
var prototype$18 = vegaUtil.inherits(Proxy, vegaDataflow.Transform);
...
var prototype$19 = vegaUtil.inherits(Relay, vegaDataflow.Transform);
...
var prototype$20 = vegaUtil.inherits(Sample, vegaDataflow.Transform);
...
var prototype$21 = vegaUtil.inherits(Sequence, vegaDataflow.Transform);
...
prototype$21.transform = function(_, pulse) {
  ...
  out.source = this.value = d3Array.range(_.start, _.stop, _.step || 1).map(vegaDataflow.ingest);
...
var prototype$22 = vegaUtil.inherits(Sieve, vegaDataflow.Transform);
...
var prototype$23 = vegaUtil.inherits(TupleIndex, vegaDataflow.Transform);
...
var prototype$24 = vegaUtil.inherits(Values, vegaDataflow.Transform);
...
var prototype$26 = WindowState.prototype;

prototype$26.init = function() {
  this.windows.forEach(function(_) { _.init(); });
  if (this.cell) this.cell.init();
};

prototype$26.update = function(w, t) {
  var self = this,
      cell = self.cell,
      wind = self.windows,
      data = w.data,
...
var prototype$25 = vegaUtil.inherits(Window, vegaDataflow.Transform);
...
prototype$25.group = function(key$$1) {
  var self = this,
      group = self.value[key$$1];

  if (!group) {
    group = self.value[key$$1] = SortedList(vegaDataflow.tupleid);
    group.stamp = -1;
  }
